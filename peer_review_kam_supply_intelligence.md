...
*Written together with the system builder after the debrief, May 2026.*
Both parties agreed with the findings presented in this report. The debrief surfaced a gap between the builder's assessment of the system's risk profile and the compliance obligations identified through independent review. The builder had correctly characterised the system as internal-facing and limited in scope, all users are employees, no customers interact with it directly, and the data processed is operationally bounded. 
That framing led to underestimating the regulatory exposure, on the assumption that an internal tool with a narrow use case carried proportionally limited obligations.

The external review made clear that internal scope does not reduce the requirements that apply when personal data is routed through external processors located in third countries. The data crosses an EEA border on every session regardless of who the end users are or how contained the use case appears from the inside. DPA and transfer mechanism obligations attach to the infrastructure, not to the perceived sensitivity of the deployment.

This is likely the most common failure mode in GDPR self-assessment of AI systems: builders correctly evaluate what the system does and who it affects at the application layer, but do not apply the same rigour to the compliance obligations generated by the underlying technical stack. An independent review is most valuable precisely because the auditor has no prior mental model of the system — and therefore evaluates the infrastructure on its own terms.